...
FIRMWARE_OBJS_IRQ = firmware/start.o firmware/irq.o firmware/print.o
...
firmware/firmware.hex: firmware/firmware.bin.irq firmware/makehex.py
	python3 firmware/makehex.py $< 16384 > $@

firmware/firmware.bin.irq: firmware/firmware.elf.irq
	$(TOOLCHAIN_PREFIX)objcopy -O binary $< $@
	chmod -x $@

firmware/firmware.elf.irq: $(FIRMWARE_OBJS_IRQ) firmware/sections.lds
	$(TOOLCHAIN_PREFIX)gcc -Os -ffreestanding -nostdlib -o $@ \
		-Wl,-Bstatic,-T,firmware/sections.lds,-Map,firmware/firmware.map,--strip-debug \
		$(FIRMWARE_OBJS_IRQ) -lgcc
	chmod -x $@
